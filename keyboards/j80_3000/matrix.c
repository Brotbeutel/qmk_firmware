...
uint8_t matrix_scan(void) {
    ...
    /* DEBUG: show mcp_ready status in row 7 */
    {
        matrix_row_t dbg = mcp_ready ? 0x00FF : 0xFF00;
        if (raw_matrix[7] != dbg) { raw_matrix[7] = dbg; changed = true; }
    }
    ...
    /* DEBUG: show raw GPIOB cache in row 6 so we can see it in keytester */
    if (mcp_ready) {
        matrix_row_t debug_val = (matrix_row_t)(~mcp_gpiob_cache & 0xFF) << 6;
        if (raw_matrix[6] != debug_val) {
            raw_matrix[6] = debug_val;
            changed = true;
        }
    }
